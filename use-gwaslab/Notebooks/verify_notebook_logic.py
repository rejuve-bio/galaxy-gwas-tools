...
dst_vcf = os.path.join(ref_dir_workspace, "ref-chr7.vcf.gz")
dst_tbi = os.path.join(ref_dir_workspace, "ref-chr7.vcf.gz.tbi")
dst_parquet = os.path.join(ref_dir_workspace, "ref-chr7.parquet")
...
cmd = f"nextflow run EBISPOT/gwas-sumstats-harmoniser -r v1.1.10 -profile docker --harm --file {target_input} --ref {ref_dir_workspace} --chrom 7 --to_build 38 --threshold 0.99"
print(f"Running: {cmd}")
